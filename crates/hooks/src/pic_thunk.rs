...
impl ThunkRegister {
    fn from_modrm_reg_field(field: u8) -> Result<Self, HookError> {
        match field {
            0 => Ok(Self::Eax),
            1 => Ok(Self::Ecx),
            2 => Ok(Self::Edx),
            ...
            6 => Ok(Self::Esi),
            7 => Ok(Self::Edi),
            _ => Err(HookError::UnsupportedThunkRegister(field)),
        }
    }
...
) -> Result<PicThunkCallSite, HookError> {
...
pub fn apply_repair_to_buffer(
    ...
    buffer[offset..offset + 5].copy_from_slice(&plan.patch_bytes);
    Ok(())
}
...
mod tests {
    ...
    fn reports_no_thunk_in_empty_buffer() {
        ...
        assert_eq!(result, Err(HookError::NoPicThunkFound));
    }
